			...
			public void actionPerformed(ActionEvent e) {
				if(projectboxCombo.getItemAt(projectboxCombo.getSelectedIndex())!=null && (projectboxCombo.getItemAt(projectboxCombo.getSelectedIndex())).toString().trim().length()!=0
					...
					try{
						...
						Project project=(Project) projectList.get(projectboxCombo.getSelectedIndex());
						myDatabaseHelper.assignEmployee((projectboxCombo.getItemAt(projectboxCombo.getSelectedIndex())).toString().trim(),(developerCombo.getItemAt(developerCombo.getSelectedIndex())).toString().trim(),project.getClient());
						currentGUIFrame.getContentPane().removeAll();
						currentGUIFrame.getContentPane().add(new EmployeesPanel(currentGUIFrame,sessionManager));
						currentGUIFrame.getContentPane().repaint();
					}catch(Exception ex){

...
public class EditCompanyPanel extends JPanel {
	...
	private JButton company,clients,projects,employees,invoices,reports,updatecompany,back,exitButton,help;
		...
		updatecompany.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(name!=null && name.getText().trim().length()!=0 && addressline1!=null && addressline1.getText().trim().length()!=0
						...
						&& state.getText()!=null && state.getText().trim().length()!=0 && zip.getText()!=null && zip.getText().trim().length()!=0){
					try{						
